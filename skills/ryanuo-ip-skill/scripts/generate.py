...
读 ~/.config/ryanuo-ip-skill/config.yaml(用 scripts/illo.py init 写入,不内置任何密钥)。

  文生图:       python3 scripts/generate.py --prompt-file p.md --out out.png
  图生图(锁角色): python3 scripts/generate.py --prompt-file p.md --reference <锚点图> --out out.png
...
CONFIG_PATH = pathlib.Path(os.environ.get("XDG_CONFIG_HOME", str(pathlib.Path.home() / ".config"))) / "ryanuo-ip-skill" / "config.yaml"
...
def run_sync(base_url, model, api_key, body, aspect, refs, out):
    ...
    if len(refs) > 1:
        eprint(
            ...
            f"({pathlib.Path(refs[0]).name}),其余 {len(refs) - 1} 张被忽略。\n"
            f"  要多图锁角色,把 config.yaml 的 backend 设为 apimart-task(用 image_urls 支持多图)。"
        )
    last_err = None
    for attempt in (1, 2):
        try:
...
def main():
    ...
    # 测真实尺寸 + 按 magic bytes 纠正后缀。上游看到 width/height,发现尺寸跑偏可据此重生。
