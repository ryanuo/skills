...
description: 微信公众号技术文章全流程：分析参考页面 → 出计划确认 → 去 AI 味写作（完整步骤/插件展示/官方链接）→ ryanuo-ip-skill 占位图 prompt 内嵌 → yuwen-publish-precheck 审核 → 可选同步 ryanuo.cc → 交付公众号版 md。Use when 用户说"写一篇微信公众号文章/公众号文章/微信文章"，尤其基于某个参考页面 URL 或主题。
version: 0.1.0
---

# 微信公众号技术文章工作流

把"基于参考页面/主题写一篇公众号文章"从分析到交付的全流程。
...
**洗稿红线**：不照抄官方文档的结构顺序（安装→示例→包含内容）。用自己的叙事结构（痛点切入→完整实操→工具展示→进阶玩法→个人观点），内容是原创场景化改写。
...
- **结构大纲**：开头痛点 → 完整步骤（含易漏的"第 0 步"）→ 工具/插件展示 → 进阶 → 个人观点
- **目标字数**（一般 2800-3500）
- **占位图清单**：位置/类型/比例/角色动作
...
- 代码块前 2-3 句口语化"为什么这样做"，代码块后补踩坑经验
- 文末互动引导（如"你平时用什么工具…评论区聊聊"）

**内容检查清单（用户常见期望，写时逐项核对）**：
- [ ] 完整步骤（step by step，含第 0 步"先定主题"这类被忽略的环节）
- [ ] 工具/插件展示和使用环节（如 VS Code 插件：安装→界面→功能→配置）
- [ ] 用户要求"增加链接"时：文末放官方资源链接列表（官网/文档/仓库/插件市场），只放官方链接
- [ ] 占位图位置预留
- [ ] 表格（技能覆盖、工具清单类内容用表格呈现）

### 3. 占位图 prompt（ryanuo-ip-skill）

加载 `personal-ip-skill` 获取形象锁定段唯一真源，读 `/Users/ryanuo/dev/skills/skills/ryanuo-ip-skill/SKILL.md`：
...
- **真实 UI 截图（VSCode 插件界面等）不用 AI 生图**——AI 画真实界面必失真，留"截图占位"说明让用户本地截
...
- **重点查 MY-01（防 AIGC 误判）**：标题痛点、开头个人化、代码块前后解释、文末互动引导——缺哪项补哪项
- 报告存档 `data/history/YYYY-MM-DD-标题.md`
- 公众号偏好：避免外部链接（尤其 X/Twitter）；用户明确要求保留链接时只放官方链接

### 5. 同步 ryanuo.cc（用户要求时）

加载 `personal-site-i18n`。文章放 `pages/zh/posts/<slug>.md`：

- frontmatter：`title` / `date: "YYYY-MM-DD HH:MM"`（带引号）/ `cate: 技术` / `description` / `plum: true`
- 正文 `[[toc]]` 开头；**正文无 H1**（H1 来自 title，站点工具会报 "Unexpected additional H1"）
- 技术文章 zh-only，不配 en pair
- 图片占位用 HTML 注释 `<!-- 图片占位 N：… -->` 内嵌 prompt 占位块（引用块+代码块），用户生图后直接替换
...
- 占位图数量按文章结构定：封面 1 + 结构图 1-2 + 对比图 + 结尾情绪图，别贪多
